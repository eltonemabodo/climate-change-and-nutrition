...
zdhs_2010_11 <- read_dta("GitHub/climate-change-and-nutrition/data /raw data/zdhs_2010:11.DTA") %>% 
  ...
  mutate(sample_weight = sample_weight/1000000,
         ...
         haz = if_else(haz == 9999, NA, haz),
         waz = if_else(waz == 9999, NA, waz),
         whz = if_else(whz == 9999, NA, whz),
...
zdhs_2010_11_survey <- svydesign(
  ids = ~zdhs_2010_11$psu,
  strata = ~zdhs_2010_11$strata_design,
  weights = ~zdhs_2010_11$sample_weight,
  data = zdhs_2010_11,
  nest = TRUE
)

# Set survey for tidyverse-friendly survey design analysis
zdhs_2010_11_survey <- as_survey_design(zdhs_2010_11_survey)

# Controlling for scenarios where only one psu will be available
options(survey.lonely.psu = "adjust")

# Create a pseudo-panel for the data
